...
public class ASTDisplay implements Visitor<String,Object> {
	...
	private String printLines(AST x) {
    	return  "(lines: " + x.posn.start + " - " + x.posn.finish + ")";
    }
    ...
    private void show(String prefix, AST node) {
    	System.out.println(prefix + node.toString() + printLines(node));
    }
    ...
    public Object visitClassDecl(ClassDecl clas, String arg){
        show(arg, clas);
        show(indent(arg), quote(clas.name) + " classname" + printLines(clas));
    ...
    public Object visitFieldDecl(FieldDecl f, String arg){
       	show(arg, "(" + (f.isPrivate ? "private": "public") 
    			+ (f.isStatic ? " static) " :") ") + f.toString() + printLines(f));
    ...
    public Object visitMethodDecl(MethodDecl m, String arg){
       	show(arg, "(" + (m.isPrivate ? "private": "public") 
    			+ (m.isStatic ? " static) " :") ") + m.toString() + printLines(m));
    ...
    public Object visitParameterDecl(ParameterDecl pd, String arg){
        ...
        show(indent(arg), quote(pd.name) + "parametername " + printLines(pd));
        return null;
    } 
    ...
    public Object visitVarDecl(VarDecl vd, String arg){
        ...
        show(indent(arg), quote(vd.name) + " varname" + printLines(vd));
        return null;
    }
    ...
    private String showType(TypeDenoter type) {
    	String result = "";
    	try {
    		ArrayType temp = (ArrayType) type;
    		result = "Array Type: " + showType(temp.eltType);
    	} catch (ClassCastException e) {
    		
    	}
    	
    	try {
    		ClassType temp = (ClassType) type;
    		result = "Class Type: " + temp.className.spelling;
    	} catch (ClassCastException e) {
    		
    	}
    	
    	try {
    		BaseType temp = (BaseType) type;
    		result = "Base Type: " + temp.typeKind.name();
    	} catch (ClassCastException e) {
    		
    	}
    	
    	try {
    		NullType temp = (NullType) type;
    		result = "Null Type";
    	} catch (ClassCastException e) {
    ...
    public Object visitUnaryExpr(UnaryExpr expr, String arg){
        show(arg, expr + printLines(expr) + " " + showType(expr.type));
    ...
    public Object visitThisRef(ThisRef ref, String arg) {
    	show(arg,ref + printLines(ref) + " (Decl line numbers: " + ref.decl.posn.start + " - " + ref.decl.posn.finish + " )");
    	return null;
    }
    ...
    public Object visitQRef(QualRef qr, String arg) {
    	show(arg, qr + printLines(qr) + " (Decl line numbers: " + qr.decl.posn.start + " - " + qr.decl.posn.finish + " )");
    ...
    public Object visitIxRef(IxRef ir, String arg) {
    	show(arg, ir + printLines(ir) + " (Decl line numbers: " + ir.decl.posn.start + " - " + ir.decl.posn.finish + " )");
    ...
    public Object visitIdentifier(Identifier id, String arg){
    	...
    	//System.out.println(id.decl.posn.finish);
        show(arg, quote(id.spelling) + " " + id.toString() + printLines(id) + " (Decl line number: " + id.decl.posn.start + ", Decl name: " + id.decl.name + ")");
        return null;
    }
    
    public Object visitOperator(Operator op, String arg){
        show(arg, quote(op.spelling) + " " + op.toString() + printLines(op));
        return null;
    }
    
    public Object visitIntLiteral(IntLiteral num, String arg){
        show(arg, quote(num.spelling) + " " + num.toString() + printLines(num));
        return null;
    }
    
    public Object visitBooleanLiteral(BooleanLiteral bool, String arg){
        show(arg, quote(bool.spelling) + " " + bool.toString() + printLines(bool));
        return null;
    }

	@Override
	public Object visitNullLiteral(NullLiteral nullLit, String arg) {
		show(arg, quote(nullLit.spelling) + " " + nullLit.toString() + printLines(nullLit));
        return null;
	}
